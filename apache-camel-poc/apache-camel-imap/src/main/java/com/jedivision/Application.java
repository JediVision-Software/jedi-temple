...
public class Application {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String address = ctx.getEnvironment().getProperty("application.imap.email.address");
        String password = ctx.getEnvironment().getProperty("application.imap.email.password");
        try {
            context.addRoutes(new RouteBuilder() {
                @Override
                public void configure() {
                    from("imaps://imap.gmail.com?username=" + address + "&password=" + password)
